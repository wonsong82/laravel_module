...
use Module\Company\Requests\PaytermRequest as StoreRequest;
use Module\Company\Requests\PaytermRequest as UpdateRequest;



class PaytermCrudController extends CrudController
{
    use CompanyCrudTrait, CustomCrudTrait;

    public function setup()
    {
        $this->crud->setModel('Module\Company\Payterm');
        $this->crud->setRouteName('company::crud.payterm');
        $this->crud->setEntityNameStrings(
            __('company::payterm.name'),
            __('company::payterm.name_plural')
        );
        ...
        $this->crud->addColumns([
            [
                'name' => 'status',
                'type' => 'constant',
                'label' => __('company::payterm.field.status'),
                'orderable' => true,
            ],
            [
                'name' => 'code',
                'label' => __('company::payterm.field.code'),
                'orderable' => true,
                'searchLogic' => $this->customSearchLogic
            ],
            [
                'name' => 'name',
                'label' => __('company::payterm.field.name'),
                'orderable' => true,
                'searchLogic' => $this->customSearchLogic
            ],
            [
                'name' => 'desc',
                'label' => __('company::payterm.field.desc'),
                'orderable' => true,
                'searchLogic' => $this->customSearchLogic
            ]
        ]);
        ...
        $this->crud->addFields([
            [
                ...
                'options' => Constant::getOptions('PAYTERM_STATUS'),
                'attributes' => [
                    'placeholder' => __('company::payterm.field.status')
                ],
            ],
            [
                'name' => 'code',
                ...
                'type' => 'text',
                'attributes' => [
                    'placeholder' => __('company::payterm.field.code'),
                    'readonly' => 'readonly'
                ],
            ],
            [
                ...
                'type' => 'text',
                'attributes' => [
                    'placeholder' => __('company::payterm.field.name')
                ],
            ],
            [
                'name' => 'desc',
                ...
                'type' => 'text',
                'attributes' => [
                    'placeholder' => __('company::payterm.field.desc')
                ],
            ]
        ]);
        ...
        // BUTTONS
        $this->crud->addButton('line', 'toggle_active_btn', 'view', 'company::admin.payterm.buttons.toggle_active', 'beginning');
    ...
    public function index()
    {
        ...
        $this->data['logs'] = app(ActivityLogController::class)->getLogs(Payterm::class);

        $this->crud->setListView('application::crud.custom.list');
        // $this->crud->setListView('test_module::admin.hello_world.list');
    ...
    public function show($id)
    {
        ...
        $this->data['logs'] = app(ActivityLogController::class)->getLogs(Payterm::class, $id);
        ...
        // $this->crud->setShowView('company::admin.payterm.show');
    ...
    public function create()
    {
        ...
        // $this->crud->setCreateView('company::admin.payterm.create');
    ...
    public function store(StoreRequest $request)
    {
        ...
        $data = $request->all();
        $controller = app('Module\Company\Controllers\Logic\PaytermController');
    ...
    public function edit($id)
    {
        ...
        // $this->crud->setEditView('company::admin.payterm.edit');
    ...
    public function update(UpdateRequest $request)
    {
        ...
        $data = $request->all();
        $item = $this->crud->model->findOrFail($request->route('payterm'));
        $controller = app('Module\Company\Controllers\Logic\PaytermController');
        $changed = true;
        try {
            $this->beginTransaction();
    ...
    {
        return (object)[
            'main' => __('company::payterm.tab.main')
        ];
    }
    ...
    public function activate(Payterm $payterm)
    {
        ...
        try {
            $this->beginTransaction();
            $item = $controller->activate($payterm);
            $this->commitTransaction();
        }
        catch(NotChangedException $e){
            $changed = false;
        ...
        return redirect()->route('company::crud.payterm.index');
    }
    ...
    public function deactivate(Payterm $payterm)
    {
        ...
        try {
            $this->beginTransaction();
            $item = $controller->deactivate($payterm);
            $this->commitTransaction();
        }
        catch(NotChangedException $e){
            $changed = false;
