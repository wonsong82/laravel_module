...
@section('header')
    <section class="content-header">
        <h1>
            ...
            <small>{{ trans('backpack::crud.add', ['name' => $crud->entity_name])}}</small>
        </h1>
        <ol class="breadcrumb">
            <li><a href="{{ url(config('backpack.base.route_prefix'), 'dashboard') }}">{{ trans('backpack::crud.admin') }}</a></li>
            <li><a href="{{ url($crud->route) }}" class="text-capitalize">{{ $crud->entity_name_plural }}</a></li>
            <li class="active">{{ trans('backpack::crud.add', ['name' => $crud->entity_name]) }}</li>
        </ol>
    </section>
@endsection
...
@section('content')
    <div class="row">
        <div class="col-md-8 col-md-offset-2">
            ...
            @if ($crud->hasAccess('list') && !($crud->popup['create'] ?? null))
            ...
            >
                ...
                <div class="box">
                    ...
                    <div class="box-header with-border">
                        <h3 class="box-title">{{ trans('backpack::crud.add_a_new', ['name' => $crud->entity_name]) }}</h3>
                    </div>
    ...
    <script>
        jQuery('document').ready(function($){
            ...
            var focusField = $('form').find('input, textarea, select').not('[type="hidden"]').eq(0),
                    ...
                    fieldOffset = focusField.offset() && focusField.offset().top,
